    ...
    <div className="bg-cream border border-earth/10 p-8 md:p-12 space-y-12 shadow-sm min-h-[500px]">
      <div className="space-y-6 border-b border-earth/20 pb-8">
        ...
        <div className="flex flex-col md:flex-row md:justify-between md:items-end gap-4">
          <div>
            <h1 className="text-2xl font-serif text-earth uppercase tracking-widest">Order Details</h1>
            <p className="font-mono text-bronze mt-2">#{order.id.slice(-8).toUpperCase()}</p>
          </div>
          <div className="text-right">
             <p className="text-sm text-earth/70 font-light mb-1">Placed on {format(new Date(order.createdAt), "MMMM d, yyyy")}</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
        <div className="md:col-span-2 space-y-8">
          <h3 className="text-sm uppercase tracking-widest text-earth font-medium border-b border-earth/10 pb-2">Items Ordered</h3>
            ...
            {order.orderItems.map((item) => (
              <div key={item.id} className="flex gap-6">
                ...
                <div className="flex-1 flex flex-col justify-center">
                  ...
                  <p className="text-sm text-earth/70 mt-1">Size: {item.variant.size || "Standard"}</p>
                  <p className="text-sm text-earth mt-2">Qty: {item.quantity}</p>
                </div>
        ...
        <div className="space-y-8 bg-sand/30 p-6 md:p-8 border border-earth/5 h-fit">
          <h3 className="text-sm uppercase tracking-widest text-earth font-medium pb-2">Order Summary</h3>
          ...
          <div className="pt-6 border-t border-earth/10">
            <h4 className="text-xs uppercase tracking-widest text-earth/50 mb-3">Shipping Detail</h4>
