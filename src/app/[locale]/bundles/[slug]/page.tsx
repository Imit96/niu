import { getBundleBySlug } from "@/app/actions/bundle-admin";
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string; slug: string }> }): Promise<Metadata> {
  ...
  const bundle = await getBundleBySlug(resolvedParams.slug, resolvedParams.locale);
...
export default async function BundleDetailPage({ params }: { params: Promise<{ locale: string; slug: string }> }) {
  const resolvedParams = await params;
  const [bundle, t] = await Promise.all([
    getBundleBySlug(resolvedParams.slug, resolvedParams.locale),
    ...
    <div className="flex flex-col w-full min-h-screen bg-sand">
      <div className="max-w-[1440px] mx-auto w-full flex flex-col lg:flex-row pt-24 min-h-[90vh]">
        ...
        <div className="w-full lg:w-1/2 p-6 lg:p-12 lg:sticky lg:top-0 h-fit">
          <div className="relative aspect-[4/5] bg-stone border border-earth/10 overflow-hidden shadow-inner">
             {bundleImage ? (
               ...
               /* Fallback: layered product images */
               <div className="absolute inset-0 flex items-center justify-center p-12">
                 <div className="relative w-full h-full">
                   ...
                   />
                   <Image
                     src={bundle.products.find(p => p.images?.[1] || p.images?.[0])?.images[0] || firstProductImage}
        ...
        <div className="w-full lg:w-1/2 p-6 lg:p-12 flex flex-col justify-center">
          <div className="space-y-6 max-w-xl">
             <div className="flex items-center space-x-4">
               <span className="text-xs font-semibold tracking-[0.2em] text-bronze uppercase">{t("curatedCollection")}</span>
               {discountPct > 0 && (
                 <span className="bg-earth text-cream text-[10px] px-2 py-1 uppercase tracking-widest">{t("setSave", { pct: discountPct })}</span>
               )}
             </div>
             ...
             <div className="pt-8 space-y-6">
                <h3 className="text-sm border-b border-earth/20 pb-2 uppercase tracking-widest text-earth/80">{t("regimenIncludes")}</h3>
